 ...
 */
void NBNS::_getnbname(char *nbname, char *name, uint8_t maxlen)
{
	uint8_t b;
	uint8_t c = 0;
 ...
 */
void NBNS::_makenbname(char *name, char *nbname, uint8_t outlen)
{
	uint8_t b;
	uint8_t c = 0;
...
bool NBNS::begin(const char *name)
{
...
void NBNS::poll(void)
{
	...
	{
		struct NBNSQUESTION question;

		_udp.read((char *)&question, sizeof(question));
		if (0 == (question.NBNSQ_FLAGS1 & 0x80))
		{
			...
			{ // dotaz primo na nas
				...
				_udp.endPacket(); // odesleme zpravu
			}
			...
			{ // obecny dotaz - mireny nejspis na nasi IP adresu
				...
				_udp.beginPacket(_udp.remoteIP(), NBNS_PORT);
				_udp.write((uint8_t *)&nbnsan, sizeof(nbnsan));
				_udp.endPacket(); // odesleme zpravu
			}
		}
		_udp.flush();
